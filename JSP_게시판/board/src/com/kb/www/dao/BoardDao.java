...
public class BoardDao {
	...
	public ArrayList<ArticleVo> getArticleList(Pagenation pagenation, String query) {
		...
		try {
			pstmt = con.prepareStatement(
					"select b.num" + ", m.id" + ", b.subject" + ", b.content" + ", b.hit" + ", b.wdate" + ", b.udate"
							+ ", b.ddate" + " from board b" + " inner join member m on b.mb_sq = m.sq" + " where 1=1"
	...
	public ArticleVo getArticle(int num) {
		...
		try {
			pstmt = con.prepareStatement("select" + " b.num" + ", b.mb_sq" + ", b.subject" + ", b.content" + ", b.hit"
					+ ", b.wdate" + ", b.udate" + ", b.ddate" + ", m.id" + " from board b"
	...
	public int insertArticle(ArticleVo vo) {
		...
		int count = 0;
		try {
			pstmt = con.prepareStatement("insert into board(mb_sq, subject, content) value(?, ?, ?)");
	...
	public int insertMember(MemberVo vo) {
		...
		int count = 0;
		try {
			pstmt = con.prepareStatement("insert into member(id, pw, email) value(?, ?, ?)");
	...
	public MemberVo getMember(String id) {
		...
		MemberVo vo = null;
		try {
			pstmt = con.prepareStatement("select sq, id, pw, email from member where binary(id)=? and leave_fl=false");
			pstmt.setString(1, id);
			rs = pstmt.executeQuery();
			while (rs.next()) {
				vo = new MemberVo();
	...
	public int updateLoginState(MemberVo vo) {
		...
		int count = 0;
		try {
			pstmt = con.prepareStatement("update member set lgn_st=? where sq=? and leave_fl=false");
	...
	public String getWriterId(int num) {
		...
		try {
			pstmt = con.prepareStatement(
					"select" + " m.id" + " from board b" + " inner join member m on b.mb_sq = m.sq" + " where num=?");
			pstmt.setInt(1, num);
			rs = pstmt.executeQuery();
			while (rs.next()) {
				id = rs.getString("id");
	...
	public ArrayList<MemberHistoryVo> getMemberHistory(String id) {
		...
		try {
			pstmt = con.prepareStatement("select " + "mh.evt_type " + ", mh.dttm " + "from member m "
					+ "left join member_history mh on m.sq = mh.mb_sq " + "where id=?");
			pstmt.setString(1, id);
			rs = pstmt.executeQuery();
			while (rs.next()) {
	...
	public int updateMember(MemberVo vo) {
		...
		int count = 0;
		try {
			pstmt = con.prepareStatement("update member set pw=?, email=? where id=?");
